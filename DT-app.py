...
#Create a Database
conn = sqlite3.connect('Loan_Databasefile.db')
cursor = conn.cursor()
cursor.execute("""
CREATE TABLE IF NOT EXISTS Loan_Predictions(
ID INTEGER PRIMARY KEY AUTOINCREMENT,
NAME TEXT,AGE INTEGER, EXPERIENCE INTEGER,INCOME REAL,FAMILY INTEGER,CREDIT_CARD_AVG REAL,EDUCATION INTEGER,
MORTGAGE REAL,SECURITIES_ACCOUNT INTEGER, CD_ACCOUNT INTEGER,ONLINE INTEGER, CREDIT_CARD INTEGER,PREDICTION TEXT) """)
conn.commit()
...
if st.button("Predict"):
    ...
    NAME,AGE,EXPERIENCE,INCOME,FAMILY,CREDIT_CARD_AVG,EDUCATION,MORTGAGE,SECURITIES_ACCOUNT,CD_ACCOUNT,
    ONLINE,CREDIT_CARD,PREDICTION) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""",(Name,Age,Experience,Income,Family,
    CCAvg,Education,Mortgage,Securities_Account,CD_Account,Online,CreditCard,RESULT))
    conn.commit()
...
if st.checkbox("📋 Prediction History (Only Admin)"):
    ...
    if password == "1234":
        ...
        data = pd.read_sql_query("SELECT * FROM Loan_Predictions",conn)
        ...
        column = st.selectbox("Select Column",["NAME","AGE","EXPERIENCE","INCOME","FAMILY",
                                "CREDIT_CARD_AVG","EDUCATION","MORTGAGE","SECURITIES_ACCOUNT","CD_ACCOUNT",
                                "ONLINE","CREDIT_CARD","PREDICTION"])
        #Update Record

        if column == "PREDICTION":
            new_value = st.selectbox("New Value",["Approved", "Rejected"])
        else:
            new_value = st.text_input("New Value")

        if st.button("Update Record (double click)"):
            cursor.execute(
                f"UPDATE Loan_Predictions SET {column} = ? WHERE ID = ?",(new_value, record_id))
...
def footer():
    st.write("---")
    st.write("Bank Loan Approval System")
...
## Configure the background image and theme of the Streamlit app.
st.markdown("""
<style>
